 ...
 */
const RULEKINDS_IN_ORDER = ['override', 'content', 'room', 'sender', 'underride']; // The default override rules to apply when calculating actions for an event. These
...
PushProcessor.rewriteDefaultRules = function (incomingRules) {
  ...
  if (!newRules.global.override) newRules.global.override = []; // Fix default override rules
  ...
  newRules.global.override = newRules.global.override.map(r => {
    ...
    if (!defaultRule) return r; // Copy over the actions, default, and conditions. Don't touch the user's
    // preference.
